...
parse_varscan_output <- function(VCFvarscan, type_call = "somatic"){
	...
	if(type_call == "somatic"){
		...
		if (dim(vcf)[2] > 17){
			...
			# Mean Base quality calculation - base quality from ref and rev
			vcf$qual <- (vcf$ref_base_quality + vcf$alt_base_quality)/2

			# Select interestinf columns
			vcf$caller <- "varscan"

			vcf_final <- subset(vcf,select = c("caller","chrom","pos","ref","alt","qual","filter",
				"genotype","tot_depth","VAF","ref_depth","alt_depth","ref_forw","ref_rev",
				"alt_forw","alt_rev"))
	...
	if(type_call == "somatic"){
		...
		if (dim(vcf)[2] > 14){
		    ...
		    vcf$V14 <- as.numeric(vcf$V14)
		  	...
		  	colnames(vcf) <- c("chrom","pos","ref","alt","qual","filter",
			"genotype","base_quality","ref_depth","alt_depth","VAF","alt_forw","alt_rev",
			"ref_forw","ref_rev","tot_depth")

		  	vcf <- vcf %>% separate(base_quality,into = c("ref_base_quality","alt_base_quality"),sep = ",") 
		  	vcf$ref_base_quality <- as.numeric(as.character(vcf$ref_base_quality))
		  	vcf$alt_base_quality <- as.numeric(as.character(vcf$alt_base_quality))
		  	vcf$qual_ref = ifelse(vcf$ref_depth == 0,0,vcf$ref_base_quality/(vcf$ref_depth))
		  	vcf$qual_alt= ifelse(vcf$alt_depth == 0,0,vcf$alt_base_quality/(vcf$alt_depth))
		  	vcf$qual = (vcf$qual_ref+vcf$qual_alt)/2
		  	vcf$qual_ref <- NULL
		  	vcf$qual_alt <- NULL
...
parse_vardict_output <- function(VEPvardict){
  ...
  # Subset columns to be consistent with the rest of the callers
  # QUAL.1 is the quality in the info fields which corresponds to "Mean quality score in reads"
  tsv_sub <- subset(tsv,select=c("Location","caller","X.CHROM","POS","REF","ALT","QUAL.1","FILTER","genotype",
                                 "DP","AF","ADJAF","VD","REFBIAS","VARBIAS","CSQ"))
  tsv_sub <-  tsv_sub  %>% 
  ...
  colnames(tsv_sub) <- c("Location","caller","chrom","pos","ref","alt","qual","filter","genotype",
                         "tot_depth","VAF","ADJVAF_ADJ_indels","alt_depth",
                         "ref_forw","ref_rev","alt_forw","alt_rev","INFO_VEP","ref_depth")
  
  tsv_final <- subset(tsv_sub,select = c("Location","caller","chrom","pos","ref","alt","qual","filter",
                                         "genotype","tot_depth","VAF","ADJVAF_ADJ_indels","ref_depth",
                                         "alt_depth","ref_forw","ref_rev","alt_forw","alt_rev","INFO_VEP"))
...
vep_expand <- function(var, VEP_fields){
  ...
  if( is.na(check_presence_variants) ){
    ...
    stop(paste0("VEP file ", info_vep_var," does not contain variants. Rerun VEP or check output."), call.=FALSE)
...
option_list = list(
	...
	make_option(c("--VEPcall"), type = "character", default = NA,
		help = "Call for VEP and cache directory."),
	...
	make_option(c("--RlibPath"), type = "character", default = "",
...
fieldsVCF <- ifelse(file.exists(vcf_input),
	file.path(dirname(vcf_input),"annotated_variants",paste0(sampleName,"_",type_call,"_main_fields.txt")),
	file.path(outvariants,caller,region_call,"annotated_variants",paste0(sampleName,"_",type_call,"_main_fields.txt")))
...
combinedVepVcf <- ifelse(file.exists(vcf_input),
	file.path(dirname(vcf_input),"annotated_variants",paste0(sampleName,"_",type_call,"_final.txt")),
...
if(opt$force){
	...
	annotate <- TRUE
	parse <- ifelse(is.na(opt$VEPcall), FALSE, TRUE)
}
...
if ( parse ) {
	...
	print(paste0("Combine VEP and VCF output"))
	...
	if ( nvariants > 200 ) {
		...
		expand_variants <- foreach(nline = seq_split, 
		    ...
		    .multicombine = TRUE)  %dopar%  {
				...
				vep_expan_vcfs <- apply(merge_VCF_VEP[start:end,], 1, vep_expand ,
					VEP_fields = info_names_string)	
				data.frame(do.call(rbind,vep_expan_vcfs))
	...
	print(combinedVepVcf)
	print(head(expand_variants))
